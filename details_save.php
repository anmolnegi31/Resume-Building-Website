...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file)) {
            $photo = $target_file;
            rename($photo, "uploads/$user_id.jpg");
            $photo="uploads/$user_id.jpg";
        } else {
    ...
    if (empty($errors)) {
        $sql = "INSERT INTO user_resumes (id, first_name, last_name, city, country, pincode, phone, email, job_title, college_name, college_location, degree, field_of_study, graduation_year, company_name, company_city, Year_of_experience, Proficient_in ,additional_details, photo) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssssssssssssssss", $user_id, $first_name, $last_name, $city, $country, $pincode, $phone, $email, $job_title, $college_name, $college_location, $degree, $field_of_study, $graduation_year, $company_name, $company_city, $Year_of_experience, $Proficient_in,$additional_details, $photo);
        $stmt->execute();
        $stmt->close();
        ...
        if ($temp_no==0){
            header("Location: template.php?id=$user_id");
        }
        else{
            header("Location: ./Templates/Template_$temp_no.php?id=$user_id");
        }
        exit;
    } else {
